...
#    logits = tf.contrib.layers.flatten(inputs = pre_logits)
...
#    loss = tf.reduce_mean(tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=labels), axis=1))
